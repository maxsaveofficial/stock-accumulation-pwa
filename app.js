...
function render(){
  ...
  $('detail').innerHTML=s!=='ALL'?'<div>Memuat detail…</div>':'Klik saham pada Pareto untuk melihat detail.';
...
function refresh(){const el=$('ticker');if(!el)return;const current=[...selectedTickers];const all=!current.length||current.length===data.length;el.innerHTML='<option value="ALL">ALL SAHAM</option>'+data.map(x=>'<option value="'+esc(x.ticker)+'">'+esc(x.ticker)+'</option>').join('');el.value=all?'ALL':(current[0]||'ALL');el.onchange=e=>{selectedTickers=e.target.value==='ALL'?new Set(data.map(x=>x.ticker)):new Set([e.target.value]);autoLoad()}}
