const $=id=>document.getElementById(id);let data=[],brokerRows=[],liveEnabled=false,selectedTickers=new Set(),backtestSeries=null,backtestTicker='';const BACKEND_URL=window.STOCKFLOW_BACKEND_URL||'https://stock-accumulation-pwa.maxsaveofficial.deno.net',PROVIDER_KEY='stockflow-provider',CACHE_KEY='stockflow-auto-cache-v1';
    ...
    try{
      ...
      setTimeout(async()=>{
        ...
        for(const h of horizons){
          ...
          try{
            const x=window.StockFlowCalibration?.walkForward(btPool,{lookback:Math.min(lb,20),horizon:h,costPct:1});
            await update(h,x);
          }catch(err){
